...
2. If the content is below the three dashes, it uses [markdown syntax](https://www.markdownguide.org/basic-syntax/).
...
4. In `src/i18n-config.js`, add the new language to resources. See the example below:
...
5. If the language you are adding is not one of the designated citywide languages under Local Law 30 and is not already included in `src/constants/languages.js`, add the language name and [language code](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes) as key value pairs of an object in the languages array, as follows:
...
2. Add the label for the link in each translation file in `src/locales`, corresponding to the label location in the previous step.
